...
function Group-DevicesByModel {
    ...
    param (
        ...
        [string]
        $DeviceType = "Input"  # Tipo de dispositivo de entrada
    )
    ...
    process {
        ...
        try {
            ...
            # Verifique se ha pelo menos um dispositivo antes de calcular o progresso
            $totalDevices = 0
            if ($modelGroups.Count -gt 0) {
                $totalDevices = ($modelGroups.Group.($DeviceType + "Folder").($DeviceType + "s")).Count
            }
            Write-Host "TotalDevices: $totalDevices, DevicesProcessed: $devicesProcessed"
            ...
            foreach ($group in $modelGroups) {
                ...
                # Modifica��o aqui: Formato do nome da pasta de modelo
                ...
                foreach ($device in $devices) {
                    Write-Host "Device: $($device.Name), Type: $($device.DeviceType)"
            ...
            Write-Host "Erro durante a execucao da funçao Group-DevicesByModel: $_"
        } finally {
...
$rootGroupName = "MANUTENCAO"
$baseGroupPath = "/$rootGroupName"
Group-DevicesByModel -BaseGroupPath $baseGroupPath -DeviceType "Input"